...
class Busca(ListaProdutos):
    def get_queryset(self, *args, **kwargs):
        termo = self.request.GET.get('termo') or self.request.session['termo'] #vai retornar o termo (o que foi escrito
        # no campo de busca), ou a sessão, caso não tenha, apenas retorna vazia

        qs = super().get_queryset(*args, **kwargs) #neste caso, estamos reutilizando e reescrevendo tudo que foi feito
        # na variavel qs no metodo get_queryset da classe PostIndex, por isso não precisa redigitar tudo como antes.
