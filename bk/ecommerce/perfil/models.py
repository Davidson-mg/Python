...
class Perfil(models.Model):
    ...
    def clean(self): #Vamos criar nossas validações dos campos cpf e cep
        ...
        cpf_enviado = self.cpf or None #pegando o cpf que já existe na base de dados
        cpf_salvo = None

        perfil = Perfil.objects.filter(cpf=cpf_enviado).first() #estamos selecionando um perfil com base no cpf do BD

        if perfil: #Se a variavel não está vazia, é pq conseguimos pergar um perfil com base nela
            cpf_salvo = perfil.cpf

            #primeiro verificamos se o cpf não é vazio e depois verificando se o usuario está atualizando o perfil.
            #Se não, quer dizer que o cpf já existe na base de dados e o usuario tá tentando usar o cpf de outro cadastro
            #por isso estamos verificando se a pk atual é igual a pk vinda do perfil da variavel perfil
            if cpf_salvo is not None and self.pk != perfil.pk: #Agora estamos checando se o cpf salva é do perfil
    ...
    #isso automaticamente e mostre nome errado, tipo, 'variacaos'
        verbose_name = 'Perfil' #essas duas variaveis são genericas do django
